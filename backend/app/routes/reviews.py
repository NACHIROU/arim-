...
async def create_review(
    review_data: ReviewCreate, current_user: UserOut = Depends(get_current_user)
):
    """
    ...
    existing_review = await reviews.find_one(
        {"shop_id": ObjectId(review_data.shop_id), "user_id": ObjectId(current_user.id)}
    )
    if existing_review:
        raise HTTPException(
            status_code=400, detail="Vous avez déjà laissé un avis sur cette boutique."
        )
...
async def get_reviews_for_shop(shop_id: str):
    ...
    review_list = await cursor.to_list(
        length=100
    )  # On limite à 100 avis pour la performance
...
async def delete_review(
    review_id: str, current_user: UserOut = Depends(get_current_user)
):
    """
